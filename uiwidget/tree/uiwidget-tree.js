...
//			$.info(sw);
		...
		,getNode : function(el){
			var id = el[0].className.split(" ")[1].split("-")[2];
//			$.debug("node id="+id);
	        if(id){
...
//			return this.getNode(this.getContainerNode().el.find('span.tree-node-selected').parent());
	...
	/**
	 * Tree Node : one of the tree item
	 */
	...
	$.uiwidget.TreeNode.prototype = {
		...
		,init : function(){
//			this.showIcons = (this.showIcons !== undefined) ? this.showIcons : (this.tree ? this.tree.showIcons : true) ;
			this.childNodes =  [];
...
//			this.ajaxLoadPage = this.tree.ajaxLoadPage;
		...
		//create node html from data, return html array
		,createNodeArray : function(){
			var data = this.data;
//			var tn = ['<div class="tree-node"', ' id="tree-node-', data.id, '"', data.disabled ? ' tree-node-disabled' : '',  '" title="', data.title, '">'];
			...
			tn.push('</span><a class="');
			tn.push(this.buildExpandIcon().join(" "));
			tn.push('"/>');
			
			if(this.tree ? this.tree.showIcons : true){
				if(data.children || !this.isLeaf()){
					tn.push('<a class="tree-node-icon tree-node-folder', this.isContainer ? ' tree-node-root' : '' ,'"/>'); 
				}else{
					tn.push('<a class="tree-node-icon tree-node-leaf"/>'); 
				}
			}
			...
			tn.push('<div class="tree-node-text" unselectable="on">', data.text, '</div></li>');	
			return tn;
	    }

		,getEl : function(){
//			this.el || (this.el = this.tree.el.find('div[tree-node-id='+this.data.id+']'));
			if(!this.el){
//				this.el =  (this.tree.el.find('div.tree-node-'+this.data.id));
//				this.el =  $(['#tree-node-',this.data.id].join(""));
//				$.debug("caller" + this.getEl.caller)
//				$.debug("this.data.id="+this.data.id);
				this.el =  this.tree.el.find(['.tree-node-',this.data.id].join(""));
//				$.debug('this.el='+this.el.length);
			}
			return this.el;
		}
		...
		*/
		...
		,buildIntent : function(){
			...
			while(p){
                if(!p.isContainer){
                    if(p.isLast()) {
						tn.unshift('<a class="tree-blank"/>');
                    } else {
						tn.unshift('<a class="tree-blank tree-line"/>');
                    }
                }
                p = p.parentNode;
            }
...
//	    	$.debug(this.appendChildByData.caller);
...
//					$.debug("tn="+tn.length);
//					$.debug("createNodeArray="+ treeNode.data.id + treeNode.createNodeArray().length);
				}
...
//				});
	    ...
	    ,getCt : function(){
	    	if(!this.ct || this.ct.length == 0){
				this.ct = $('<ul class="tree-node-ct"></ul>');
				if(this.isContainer)
