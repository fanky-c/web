	...
	$.uiwidget.Grid.prototype = {
		...
		*/
		...
		,buildHeadRow : function(cm){
			...
			$.each(cm, function(i, n){
				if(n.checkbox){
					...
					headRow.push('<td ', n.row?' rowspan='+n.row:'', n.col?' colspan='+n.col:'', n.hidden?' style="display:none"':'', ' width="', n.width,'"  class="grid-cm-', n.dataIndex,' grid-header-', n.dataIndex, '">');
					if(i == 0){ 
						headRow.push('<span class="grid-title-blank" title="', n.header, '" unselectable="on">', n.header);
					}else{
						headRow.push('<span class="grid-title-line" title="', n.header, '" unselectable="on">', n.header);
					}
					if(n.iconCls){
						headRow.push('<input type="button" class="', n.iconCls, '" onclick="return false;">');
					}
					if(n.sortable !== false){
						headRow.push('<input type="button" class="sort-icon" onclick="return false;"/>');
					}
					headRow.push('</span></td>');
				}
			});
			return 	headRow;
		}
		...
		,renderEditor : function(){
			var t = this;
			//render focus
			t.focus = $('<input class="grid-focus" tabindex="1" type="text"/>');
			t.mainBodyWrap.append(t.focus);
			$.each(this.cm, function(i, n){
				if(n.editor ){
					if(typeof n.editor == 'string'){
						n.editor = t.mainBodyWrap.editor({field: n.editor});
					}else if(typeof n.editor == 'object'){
						var cfg = n.editor;
						if(cfg.type){
							if(cfg.type == 'text'){
		...
		,doFocus : function(bindEl){
			this.focus.css('top', bindEl.position().top);
			this.focus.focus();
		}
		...
		,getDataIndex : function(element){
			return element.className.split(' ')[0].split('-')[2];
		}
		/**
		加载url数据
		...
		*/
		,modifySelectedRecords : function(records){
			...
			for(var i = 0, len = records.length; i < len ; i ++){
				...
				//var index = this.grid.find('table.grid-main tbody tr').index(row);
		...
		,renderData : function(){
			...
			if(this.data && this.data.result){
				...
				for(var i = 0 ; i < this.data.result.length ; i ++){
					rowArr = rowArr.concat(this.buildRow(i, this.data.result[i]));
				}
			...
			this.resizeMainBody();
			this.calcMainBodyHeight();
			this.triggerHandler('afterRender', [this]);
		}
		//解决FF有横行滚动条时，行选择效果显示不全
		,resizeMainBody : function(){
			if(this.viewConfig && this.viewConfig.horizontalScroll){
				var mainTable = this.mainBody.find('.grid-main');
				if(mainTable.length > 0)
					this.mainBody.width(mainTable.width());
			}
		}
		//计算mainBody高度，判断是否要加scrollOffset
		,calcMainBodyHeight : function(){
			if(this.mainBody.height() > this.mainBodyWrap.height()){
				this.addScrollOffset();
			}else{
				this.removeScrollOffset();
			}
		}
		//添加纵向滚动条右边的空白偏移
		,addScrollOffset : function(){
			if(!this.hasScrollOffset()){
				this.mainHeadWrap.find('.grid-main').width(this.mainBodyWrap.width() - this.scrollOffsetWidth);
		...
		//判断是否有scrollOffset
		,hasScrollOffset : function(){
			return this.mainHeadWrap.hasClass('grid-mainhead-wrap-scrollOffset');
		}
			...
			//for(var i = 0 ; i < result.length ; i ++){
				...
				for(var i = 0 ; i < this.cm.length ; i ++){
					...
					if(m.checkbox){
						...
						if(m.renderer){
							text = m.renderer(text, result, rowIndex, i, metadata, this);
						}
						btd.push('<td hidefocus="true" width="', m.width,'" class="', metadata.css, '" title="', metadata.title, '" tabindex="1">');
		...
		,showEmptyText : function(){
			this.mainBody.append(this.emptyText || (this.emptyText = $('<div class="grid-empty-text">'+ this.emptyMsg +  '</div>')));
			this.emptyText.show();
		}
		...
		,appendRow : function(data){
			...
			if(data.length){
				for(var i = 0; i < data.length; i++){
					var rowEl = $(this.buildRow(this.data.result.length + i, data[i]).join(''));
		...
		*/
		...
		,findParentCell : function(el){
			if(el.is('[class^=grid-cm]')){
				return el;
			}else{
				return el.parents('[class^=grid-cm]');
			}
		}
		,findColumModelName : function(className){
			return className.split(' ')[0].substring(8);
		}
		//
	...
	*/
	...
	$.uiwidget.CellSelectionModel.prototype = {
		...
		,select : function(cellEl){
			if(grid.triggerHandler('beforeCellSelect', [grid, cellEl]) === false){
				return;
			}
...
//			$.debug('handlerKeyDown row = ' + r + ' | col=' + c);
			if(key == 13){//enter
//				var inner = this.getSelectedCell().children('.grid-body-inner-span');
//				var title = inner.attr('title');
//				if(this.grid.cm[c].editor) 
//					this.grid.cm[c].editor.startEdit(inner, title, {left:-this.grid.cellBorder, top:-this.grid.cellBorder});
		...
		,goCells : function(r, c, step){
			//$.debug('goCells row = ' + r + ' | col=' + c);
			r == -1 ? r = 0 : r = r;
			c == -1 ? c = 0 : c = c;
			var rowEl = this.grid.getRow(r);
			if(rowEl.length < 1){
				rowEl = this.grid.getRow(r - 1);
			}
	...
	*/
	...
	$.extend($.uiwidget.EditCellSelectionModel.prototype, {
		...
		,startEditing : function(rowIndex, colIndex){
			this.stopEditing();
			var grid = this.grid;
			if(!grid.cm[colIndex].editor)
				return;
			// 处理 safari, ie 和 opera的问题
			$.defer(function(){
				this.activateEditor = grid.cm[colIndex].editor;
				var editEl = grid.getCell(rowIndex, colIndex).find('.grid-body-inner-span');
				grid.cm[colIndex].editor.startEdit(editEl, 
						grid.data.result[rowIndex][colIndex],
		...
		render : function(){
			...
			this.grid.bind('afterRender', function(e, grid){
				...
				if(grid.viewConfig && grid.viewConfig.horizontalScroll){
					grid.addHorizontalScroller(t.summaryBar);
				}
			});
		}
...
//				$.debug('>5');
		...
		/*,renderJumper : function(text, arr){
			arr = arr.concat(['<a href="javascript:void(0)" class="page" hidefocus="true"><span class="ellipsis last-ellipsis">...</span>', text, '</a>']);
			return arr;
		}*/
	...
	*/
	$.uiwidget.Calculations = {
		...
		,average : function(value, dataIndex, rowIndex, data){
			var v = Number(data[dataIndex]);
			if(!isNaN(v)){
				//return $.accDiv(($.accAdd($.accMul((value||0) , rowIndex) , v)), (rowIndex+1));
