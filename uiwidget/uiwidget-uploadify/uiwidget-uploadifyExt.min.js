...
(function($) {
	...
	$.uiwidget.UploadifyExt.prototype = {
		...
		init : function(cfg) {
			...
			var html = '			<table class="uploadifyExt-main" cellspacing="0" cellpadding="0" border="0"><tbody><tr>';
			$.each(t.headers, function(i, n) {
				...
				html += '	<th width="' + n.width + '" class="' + className
						+ '">' + n.text + '</th>'
			});
			html += '</tr></tbody></table>';
			t.target.html(html);
			...
			$('#' + t.buttonPosition)
					.append(
							'<input type="file" name="uploadify" id="' + t.uploadifyId + '"/>');
			...
			$.extend(cfg,
					{
						onSelect : function(event, queueID, fileObj) {
							if (t.fileExt) {
								if (t.fileExt.indexOf(fileObj.type
										.toLowerCase()) == -1) {
							...
							if (t.countLimit
									&& t.countLimit <= t.queueIDs.length) {
							...
							cfg,
							{
								onError : function(event, queueID, fileObj,
									...
									$('#' + queueID)
											.find('td.state')
											.html(
													'<font name="uploadError" color="red">上传出错</font>');
		...
		render : function(event, queueID, fileObj) {
							...
							function(i, n) {
								...
								if (n.key == t.deleteKey) {
									html += '	<td width="'
											...
											+ '" class="'
											+ className
											+ '"><a title="删除" href="javascript:void(0);" class="del"></a></td>'
								} else if (n.key == t.stateKey) {
									if (fileObj.size > t.sizeLimit) {
										html += '	<td width="' + n.width
												+ '" class="' + className
												+ '"><font color="red">已超'
												+ formatLimitSize
												+ '上限</font></td>'
									...
									html += '	<td width="'
											...
											+ className
											+ '"><a style="'
											+ ((text && text != null) ? 'display:none'
													: '')
											+ '" class="uploadifyExt-click-input" href="javascript:void(0);">点击输入</a><span style="'
											+ ((text && text != null) ? ''
													: 'display:none')
											+ '" title="'
											+ text
											+ '" class="uploadifyExt-inner-span">'
											+ text + '</span>';
									if (n.editAble) {
			...
			$('#' + queueID).find('.uploadifyExt-input-text').blur(
					function(e) {
			...
			$('#' + queueID).find('.uploadifyExt-input-text').keyup(
					function(e) {
						if (e.keyCode == 13) {
							$(this).trigger('blur')
						}
					});
			$('#' + queueID).find('td a.uploadifyExt-click-input').click(
					function(e) {
		...
		changeHeight : function(queueID) {
			...
			t.target.height(t.target.find('tr').first().height() + countHeight
					* t.rowHeight)
		},
		...
		renderPercentage : function(event, queueID, fileObj, date) {
			$('#' + queueID).find('td.' + this.stateKey).html(
					date.percentage + '%')
		},
		...
		getFileEl : function() {
			return this.fileEl
					|| (this.fileEl = this.target.find('input[type=file]'))
		}
	};
